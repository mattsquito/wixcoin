...
#if defined(NDEBUG)
# error "Wixcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2e0adf8dd070bed8a6242b60e3b1bc81e158738b3111318f37d1744337aa0384");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Wixcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Wixcoin Signed Message:\n";
...
{
    // Wixcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 500000); // Wixcoin: 20k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Wixcoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Wixcoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Wixcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Wixcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8f62956a632c844053cda512260c540d297d76a616762ed195411ed64f7d858e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Man hospitalised after eating red hot chilli pepper 10/04/18";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b107530c987654b469ef5d077c9cd76c540f8c9108f5a839df77a2332df7f523becf5144fd8c8e0aefa7390849b6492ad415c8ae38ffe7d072bcad0b1bd4254e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1523366116;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085155492;

        if (fTestNet)
        {
            block.nTime    = 1523366090;
            block.nNonce   = 386574462;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x588fad9bd4736ce8c3cb0dd948c3d5e8f31722ef3bb1a0295cb574ddafc2e20d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf4, 0xc1, 0xb5, 0xd7 }; // Wixcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WixcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WixcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static WixcoinMiner(CWallet *pwallet)
{
    printf("WixcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("wixcoin-miner");
    ...
    try { loop {
        ...
        printf("Running WixcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("WixcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WixcoinMiner, pwallet));
}
